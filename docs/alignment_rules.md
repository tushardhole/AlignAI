...
AlignAI uses a two-phase approach to align resumes and cover letters to target job opportunities:
1. **Alignment Phase**: Rewrite resume/cover letter using LLM agents (single-pass or chunked based on document size)
2. **Structuring Phase**: Extract aligned content into JSON schema for PDF/HTML rendering

## Single-Pass vs Chunked Decision

The system chooses between two alignment strategies based on combined document size:
...
- **Agents**: JobAnalyst, ResumeParser, ResumeSectionAligner (parallel), ResumeMerger, Structurers (8 agents)
...
- **Output**: `JobBriefFields` with title, summary, must_have_skills (≤15), nice_to_have_skills (≤10)
- **Key Rules**:
  ...
  - Distinguish must-have (explicit requirements) from nice-to-have (mentioned as "preferred")
...
- **Key Rules**:
  ...
  - Keep 4-8 bullets per role (prefer 4-6, but don't force if all relevant)
...
- **Input**: Section heading, content, job brief, job description ([:12,000])
...
- **Output**: `StructuredResumeFields` with JSON schema (name, email, skills, experience, etc.)
- **Key Rules**:
  - Include ALL bullet points exactly as written
  - Extract EVERY Tech/Tools/Stack/Environment/Platforms line as separate meta entry
  - Preserve professional summary word-for-word, no shortening
  - Detect and extract extra sections (Open Source, Personal Projects) with correct titles
...
- **Output**: `StructuredCoverLetterFields` with candidate_name and paragraphs array
- **Key Rules**:
...
- **Key Rules**:
  ...
  - Consider keyword match density, technical terminology, formatting clarity
...
| Resume (section aligner, chunked) | 12,000 chars | Individual section context |
| Job description (merger) | 4,000 chars | Merger context (minimized) |
...
Prompts are loaded via `alignai.agents.prompts.load_prompt(name)` and `load_schema_hint(name)`.
...
- Character thresholds (update all 3 locations: code, this doc, commit message)
