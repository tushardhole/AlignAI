...
def _source_lines(node: ast.ClassDef) -> int:
    """Count non-blank, non-docstring body lines for a class node."""
    lines = 0
    ...
    for py_file in _python_files(SRC_ROOT):
        ...
        for node in ast.walk(tree):
            if isinstance(node, ast.ClassDef):
                # Count actual code lines (end_lineno - lineno as a proxy; AST walk is approximate)
    ...
    for layer in ("domain", "application"):
        ...
        for src_file, symbols in _public_symbols(layer_dir).items():
            for name in sorted(symbols):
                ...
                has_test = any(
                    t == f"test_{name}" or t.endswith(f"_{name}") for t in tested
                )
                if not has_test:
                    violations.append(
...
def check_import_linter() -> list[str]:
    result = subprocess.run(
        [sys.executable, "-m", "importlinter", "--config", "pyproject.toml"],
        capture_output=True,
        text=True,
        cwd=REPO_ROOT,
    )
